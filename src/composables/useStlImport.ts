...
function releaseAssemblyItemGeometry(item: AssemblyItem): void {
...
async function parseStlBuffer(buffer: ArrayBuffer): Promise<BufferGeometry> {
  return new Promise((resolve, reject) => {
    ...
    worker.onmessage = (event: MessageEvent<StlWorkerResponse>) => {
      ...
      resolve(buildGeometry(response.positions, response.normals));
    };
...
async function processNextInQueue(): Promise<void> {
  while (activeWorkers.value < workerLimit && fileQueue.value.length > 0) {
    const nextPath = fileQueue.value.shift();
    if (!nextPath) {
      return;
    }

    activeWorkers.value += 1;
    void processFile(nextPath);
  }
}
...
function removePart(itemId: string): void {
  ...
  const [part] = assembly.value.splice(index, 1);
  releaseAssemblyItemGeometry(part);
